...
const districtsByRegion: Record<string, string[]> = {
  "Toshkent shahri": [
    "Chilonzor", "Yunusobod", "Mirzo Ulug‘bek", "Olmazor", "Shayxontohur",
    "Yakkasaroy", "Chirchiq", "Sergeli", "Uchtepa", "Bektemir", "Mirobod",
  ],
  "Qoraqalpog‘iston Respublikasi": [
    "Nukus shahri", "Amudaryo", "Beruniy", "Chimboy", "Ellikqala", "Kegeyli",
  ...
  Andijon: [
    ...
    "Jalaquduq", "Marhamat", "Oltinko‘l", "Paxtaobod", "Ulug‘nor", "Xo‘jaobod",
  ],
  Buxoro: [
  ...
  "Farg‘ona": [
    "Farg‘ona shahri", "Altiariq", "Bag‘dod", "Beshariq", "Dang‘ara",
    "Farg‘ona tumani", "Furqat", "Qo‘shtepa", "Rishton", "So‘x",
    "Uchko‘prik", "Yozyovon",
  ],
  Jizzax: [
  ...
  Xorazm: [
    "Urganch shahri", "Bog‘ot", "Gurlan", "Qoshko‘pir", "Shovot",
    "Urganch tumani", "Xazorasp", "Xiva", "Yangiariq", "Yangibozor",
  ],
  Namangan: [
    "Namangan shahri", "Chortoq", "Chust", "Kosonsoy", "Mingbuloq",
    "Namangan tumani", "Norin", "Pop", "To‘raqo‘rg‘on", "Uchqo‘rg‘on", "Yangiqo‘rg‘on",
  ],
  Navoiy: [
  ...
  Qashqadaryo: [
    "Qarshi shahri", "Chiroqchi", "Dehqonobod", "G‘uzor", "Qamashi",
    "Qarshi tumani", "Koson", "Muborak", "Nishon", "Yakkabog‘",
  ],
  Samarqand: [
    "Samarqand shahri", "Bulung‘ur", "Ishtixon", "Jomboy", "Kattaqo‘rg‘on",
    "Narpay", "Nurobod", "Oqdaryo", "Paxtachi", "Payariq", "Urgut",
  ],
  Sirdaryo: [
  ...
  Surxondaryo: [
    "Termiz shahri", "Angor", "Boysun", "Denov", "Jarqo‘rg‘on", "Muzrabod",
    "Oltinsoy", "Qiziriq", "Sherobod", "Sho‘rchi",
  ],
  "Toshkent viloyati": [
    "Nurafshon shahri", "Angren", "Bekobod", "Bo‘ka", "Bo‘stonliq",
    "Chirchiq shahri", "Oqqo‘rg‘on", "Parkent", "Piskent", "Quyi Chirchiq", "Yuqori Chirchiq",
  ],
}
...
export function ProfileForm({
  ...
  const [profileFile, setProfileFile] = useState<File | null>(null)
  const [profileImage, setProfileImage] = useState<string | null>(initialData.photo || "/default-avatar.png")

  const handleChange = (name: string, value: string) => {
    setFormData((prev) => {
      const newData = { ...prev, [name]: value }
      if (name === "region") newData.district = ""
      return newData
    })
  }
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (
        ...
        formDataToSend.append(
          key,
          key === "type" ? reverseTypeMap[value] || value : value
        )
      }
    })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Grid: 1-2-3 ustun (mobil: 1, md: 2, lg: 3) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

        {/* 1. Tashkilot nomi + rasm */}
        <Card className="p-6 bg-gradient-to-br from-card to-card/80 backdrop-blur-md border-border/50 shadow-lg hover:shadow-xl transition-all duration-300 col-span-1 max-h-96 overflow-y-auto">
          <div className="space-y-6">
            <div className="flex flex-col items-center space-y-4">
              <ProfileImageUploader
                onImageChange={handleImageChange}
          ...
          <div className="space-y-6 px-6">
            <div className="space-y-2">
              ...
              <div className="mt-1 flex items-center gap-2">
                <p className="text-sm text-foreground">{formData.id || "N/A"}</p>
                {formData.id && (
                  <Button type="button" variant="ghost" size="sm" onClick={handleCopyId} className="hover:bg-primary">
                    <Copy className="h-4 w-4" />
                  </Button>
                )}
              </div>
            ...
            <div className="space-y-2">
              <Label className="text-lg font-medium">Qo‘shilgan vaqti</Label>
              <p className="text-sm text-foreground">{formatDateTime(formData.created_at)}</p>
            </div>
            <div className="space-y-2">
              <Label className="text-lg font-medium">O‘zgartirilgan vaqti</Label>
              <p className="text-sm text-foreground">{formatDateTime(formData.updated_at)}</p>
            </div>
          </div>
        </Card>
      ...
      {/* Saqlash tugmasi faqat tahrirlashda */}
      {/* {isEditing && (
