...
//   )
// }
// src/components/manufacturer/partners/ManufacturerPartnersPage.tsx
"use client"
...
export default function ManufacturerPartnersPage() {
  const [partners, setPartners] = useState<Partner[]>([])
  const [partnerRequests, setPartnerRequests] = useState<PartnerRequest[]>([])
  const [myPartnerRequests, setMyPartnerRequests] = useState<PartnerRequest[]>([])
  const [isAddModalOpen, setIsAddModalOpen] = useState(false)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAddError(err instanceof Error ? err.message : "Xato yuz berdi")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAddPartner = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      await addPartner({ partner: formData.partner })
      setIsAddModalOpen(false)
      setFormData({ partner: "" })
      await fetchData()
    } catch (err) {
      setAddError(err instanceof Error ? err.message : "Xato")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAcceptRequest = async (id: number) => {
    ...
    try {
      ...
      setAcceptError(err instanceof Error ? err.message : "Xato")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const getPartnerProfile = (partner: Partner) => partner.partner.id !== myId ? partner.partner : partner.owner
  const getIncomingRequestProfile = (request: PartnerRequest) => request.owner
  const getOutgoingRequestProfile = (request: PartnerRequest) => request.partner

  if (isLoading && partners.length === 0) {
    return <Card className="p-6 text-center"><p>Yuklanmoqda...</p></Card>
  }

  return (
    <div className="bg-gradient-to-b from-background to-background/90 min-h-screen p-4 sm:p-8">
      <div className="container mx-auto space-y-6">
        ...
        {acceptError && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>{acceptError}</AlertDescription></Alert>}

        {/* Mavjud ta'minotchilar */}
        <Card className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-semibold flex items-center"><Users className="mr-2" /> Mavjud ta'minotchilar</h2>
            <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
              <DialogTrigger asChild>
                <Button><Plus className="mr-2 h-4 w-4" /> Yangi ta'minotchi</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Yangi ta'minotchi qo'shish</DialogTitle></DialogHeader>
                <form onSubmit={handleAddPartner} className="space-y-4">
                  <div>
                    <Label>Ta'minotchi ID</Label>
                    <Input value={formData.partner} onChange={e => setFormData({ partner: e.target.value })} placeholder="ID kiriting" required />
                  </div>
                  <Button type="submit" disabled={isLoading}>Qo'shish</Button>
                </form>
              </DialogContent>
            </Dialog>
          </div>
          ...
          {partners.length === 0 ? (
            <p className="text-center py-8 text-gray-500">Hech qanday ta'minotchi yo'q</p>
          ) : isMobile ? (
            <div className="space-y-4">
              {partners.map(p => {
                const prof = getPartnerProfile(p)
                return (
                  <Card key={p.id} className="p-4">
                    <div><strong>{prof.name}</strong></div>
                    <div>CEO: {prof.ceo || "N/A"}</div>
                    <div>No'mer: {prof.phone || "N/A"}</div>
                    <div className="flex gap-2 mt-2">
                      <Button size="sm" variant="ghost" onClick={() => handleViewPartner(p.id)}><Info className="h-4 w-4 mr-1" /> Ko'rish</Button>
                      <Button size="sm" variant="ghost" onClick={() => { setDeletePartnerId(p.id); setIsDeleteModalOpen(true) }} className="text-red-600"><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </Card>
                )
              })}
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Nomi</TableHead><TableHead>CEO</TableHead><TableHead>No'mer</TableHead><TableHead>Amallar</TableHead></TableRow></TableHeader>
              <TableBody>
                {partners.map(p => {
                  const prof = getPartnerProfile(p)
                  return (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={() => handleViewPartner(p.id)}><Info className="h-4 w-4" /></Button>
                        <Button size="sm" variant="ghost" onClick={() => { setDeletePartnerId(p.id); setIsDeleteModalOpen(true) }} className="text-red-600"><Trash2 className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  )
        ...
        {/* Kelgan so'rovlar */}
        <Card className="p-6">
          <h3 className="text-xl font-semibold mb-4 flex items-center"><MessageSquare className="mr-2" /> Menga kelgan so‘rovlar</h3>
          {partnerRequests.length === 0 ? (
            <p className="text-center py-8 text-gray-500">So'rovlar yo'q</p>
          ) : isMobile ? (
            <div className="space-y-4">
              {partnerRequests.map(r => {
                const prof = getIncomingRequestProfile(r)
                return (
                  <Card key={r.id} className="p-4">
                    <div><strong>{prof.name}</strong></div>
                    <div>Vaqt: {formatDateTime(r.invited_at)}</div>
                    <div className="flex gap-3 mt-2">
                      <Button size="sm" onClick={() => handleAcceptRequest(r.id)}><Check className="h-4 w-4 mr-1" /> Qabul</Button>
                      <Button size="sm" variant="destructive" onClick={() => { setRejectRequestId(r.id); setIsRejectModalOpen(true) }}><X className="h-4 w-4" /></Button>
                    </div>
                  </Card>
                )
              })}
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Nomi</TableHead><TableHead>Vaqt</TableHead><TableHead>Amallar</TableHead></TableRow></TableHeader>
              <TableBody>
                {partnerRequests.map(r => {
                  const prof = getIncomingRequestProfile(r)
                  return (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="flex  gap-2">
                        <Button size="sm" onClick={() => handleAcceptRequest(r.id)}><Check className="h-4 w-4" /></Button>
                        <Button size="sm" variant="destructive" onClick={() => { setRejectRequestId(r.id); setIsRejectModalOpen(true) }}><X className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  )
        ...
        <Card className="p-6">
          <h3 className="text-xl font-semibold mb-4 flex items-center"><MessageSquare className="mr-2" /> Men yuborgan so‘rovlar</h3>
          {myPartnerRequests.length === 0 ? (
            <p className="text-center py-8 text-gray-500">So'rovlar yo'q</p>
          ) : (
            <Table>
              <TableHeader><TableRow><TableHead>Nomi</TableHead><TableHead>Vaqt</TableHead><TableHead>Amal</TableHead></TableRow></TableHeader>
              <TableBody>
                {myPartnerRequests.map(r => {
                  const prof = getOutgoingRequestProfile(r)
                  return (
                    <TableRow key={r.id}>
                      <TableCell>{prof.name}</TableCell>
                      <TableCell>{formatDateTime(r.invited_at)}</TableCell>
                      <TableCell>
                        <Button size="sm" variant="ghost" onClick={() => { setDeleteMyRequestId(r.id); setIsMyDeleteModalOpen(true) }} className="text-red-600"><Trash2 className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  )
                })}
        ...
        <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
          <DialogContent><DialogHeader><DialogTitle>O'chirish</DialogTitle></DialogHeader><p>Haqiqatan o'chirmoqchimisiz?</p><DialogFooter><Button variant="outline" onClick={() => setIsDeleteModalOpen(false)}>Bekor</Button><Button variant="destructive" onClick={handleDeletePartner}>O'chirish</Button></DialogFooter></DialogContent>
        </Dialog>

        <Dialog open={isRejectModalOpen} onOpenChange={setIsRejectModalOpen}>
          <DialogContent><DialogHeader><DialogTitle>Rad etish</DialogTitle></DialogHeader><p>So'rovni rad etasizmi?</p><DialogFooter><Button variant="outline" onClick={() => setIsRejectModalOpen(false)}>Bekor</Button><Button variant="destructive" onClick={handleRejectRequest}>Rad etish</Button></DialogFooter></DialogContent>
        </Dialog>

        <Dialog open={isMyDeleteModalOpen} onOpenChange={setIsMyDeleteModalOpen}>
          <DialogContent><DialogHeader><DialogTitle>O'chirish</DialogTitle></DialogHeader><p>So'rovni o'chirmoqchimisiz?</p><DialogFooter><Button variant="outline" onClick={() => setIsMyDeleteModalOpen(false)}>Bekor</Button><Button variant="destructive" onClick={handleDeleteMyRequest}>O'chirish</Button></DialogFooter></DialogContent>
        </Dialog>
            ...
            {selectedPartner && (() => {
              ...
              return (
                <div className="space-y-3 text-sm">
                  <div><strong>Nomi:</strong> {prof.name}</div>
                  <div><strong>CEO:</strong> {prof.ceo || "N/A"}</div>
                  <div><strong>No'mer:</strong> {prof.phone || "N/A"}</div>
                  <div><strong>Email:</strong> {prof.email || "N/A"}</div>
                  <div><strong>Taklif vaqti:</strong> {formatDateTime(selectedPartner.invited_at)}</div>
                  <div><strong>Qabul vaqti:</strong> {formatDateTime(selectedPartner.accepted_at)}</div>
                </div>
              )
            })()}
          </DialogContent>
